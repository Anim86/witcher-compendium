...
lines.forEach(line => {
    ...
    if (match) {
        ...
        let schoolMatch = name.match(/Scuola del[l']*\s+(\w+)\s+\((\w+)\)/i);
        let schoolInfo = null;
        if (schoolMatch) {
            schoolInfo = {
        ...
        if (f.name === match.fileName && f.path === targetPath) {
            ...
            try {
                ...
                if (fs.existsSync(targetPath)) {
                    // If target already exists, maybe append a hash or skip
                    // For now, let's append a small random to avoid collisions if they are different versions
                    // But usually, we want to OVERWRITE if it's the same item.
...
// 5. Final Report
fs.writeFileSync('e:/AntigravitiProgetti/CompendioTheWitcher/scratch/sanitization_report_v3.json', JSON.stringify(report, null, 2));

console.log(`\nSanitization V3 Complete (School Gear Support):`);
