...
const configs = [
    {
        dir: 'witcher-animals',
        title: 'Report Animali',
        outFile: 'report_animali.md'
    },
    {
        dir: 'witcher-characters',
        title: 'Report Personaggi',
        outFile: 'report_personaggi.md'
    }
];

const basePath = path.join(__dirname, '..', 'src-packs', 'BESTIARIO');
const outPath = path.join(__dirname, '..', '..', 'TO DO');

for (const config of configs) {
    const dataDir = path.join(basePath, config.dir);
    const outputFile = path.join(outPath, config.outFile);
    
    if (!fs.existsSync(dataDir)) {
        console.log(`Directory ${dataDir} not found, skipping.`);
        continue;
    }
    ...
    for (const file of files) {
        ...
        try {
            ...
            for (const statKey in skills) {
                ...
                if (typeof statSkills === 'object' && statSkills !== null) {
    ...
    markdown += '| Nome | Statistiche | Stat Derivate | N° Abilità (Skills) | Armi/Attacchi | Capacità (Abilities) |\n';
    markdown += '|---|---|---|---|---|---|\n';
