...
<html>
<head>
    <title>Preview Rimedi Immagini v4</title>
    <style>
...
<body>
    <h1>Anteprima Immagini v4 (Range +/- 2 Pagine)</h1>
    <p>Ricerca estesa nel range di pagine per trovare illustrazioni a tutta pagina senza nome nel file.</p>
`;

for (const m of monsters) {
    const code = m.source.split(' ')[0];
    const basePage = parseInt(m.source.split(' ')[1]);
    ...
    // Define the range [basePage - 2, basePage + 2]
    const range = [];
    for (let p = basePage - 2; p <= basePage + 2; p++) {
        range.push(p);
    }
    
    // Find all images in this range
    const rangePatterns = range.map(p => 'Pag' + p.toString().padStart(3, '0'));
    const rangePatternsShort = range.map(p => 'Pag' + p);
    
    let candidates = allFiles.filter(f => {
        const lower = f.name.toLowerCase();
        const matchesPage = rangePatterns.some(p => f.name.includes(p)) || rangePatternsShort.some(p => f.name.includes(p));
        return matchesPage;
    });

    // Selection Priority Logic
    // 1. Files containing the name (sorted by size)
    const nameMatches = candidates.filter(f => {
        const lower = f.name.toLowerCase();
        return lower.includes(cleanNameStr) || lower.includes(nameNoApostrophe);
    }).sort((a,b) => b.size - a.size);

    // 2. Others sorted by size
    const otherMatches = candidates.filter(f => !nameMatches.includes(f)).sort((a,b) => b.size - a.size);

    // Final top 3 based on priority
    // Priority 1: name matches first
    // Then fill the rest with largest files in range
    let top3 = [];
    top3.push(...nameMatches);
    top3.push(...otherMatches);
    
    // Distinct files and limit to 3
    top3 = Array.from(new Set(top3)).slice(0, 3);
    ...
    if (top3.length === 0) {
        html += `<p>NESSUNA IMMAGINE TROVATA NEL RANGE</p>`;
    } else {
        top3.forEach((c, idx) => {
...
fs.writeFileSync('scratch/immagini-remediate-preview-v4.html', html, 'utf8');
console.log('HTML v4 preview generated in scratch/immagini-remediate-preview-v4.html');
const reportPath = path.resolve('scratch/immagini-remediate-preview-v4.html');
