...
const targetSpells = [
    ...
    // Master
    "Tuono di Alzur", "Dissipazione", "Dividere le Acque", "Effetto Specchio", "Portale", "Teletrasporto"
];
...
fs.writeFileSync('e:/AntigravitiProgetti/CompendioTheWitcher/scratch/final_spells_patch.json', JSON.stringify(finalOutput, null, 2));

console.log(`Final Patch Generated: ${finalOutput.length} items.`);
const foundNames = foundItems.map(i => i.name);
console.log(`Spells included: ${foundNames.length}`);
const missing = targetSpells.filter(ts => !foundNames.some(fn => fn.toLowerCase() === ts.toLowerCase() || (fn.toLowerCase() === "sacca daria" && ts === "Sacca d'aria")));
if (missing.length > 0) console.log(`Missing spells: ${missing.join(", ")}`);
