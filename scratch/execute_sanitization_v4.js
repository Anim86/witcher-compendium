...
lines.forEach(line => {
    ...
    if (match) {
        ...
        // Handle "Scuola della Manticora (Acciaio)", "Scuola del Lupo (Acciaio)", "Scuola dell'Orso (Acciaio)"
        let schoolMatch = name.match(/Scuola\s+(?:del|della|dell'|dell)\s+(\w+)\s+\((\w+)\)/i);
        let schoolInfo = null;
        if (schoolMatch) {
            schoolInfo = {
                school: schoolMatch[1].toLowerCase(),
                type: schoolMatch[2].toLowerCase()
            };
        }
...
actualFiles.forEach(f => {
    ...
    // Special School Matching
    if (!match) {
        match = csvItems.find(item => {
            if (!item.schoolInfo) return false;
            // File must contain both school and type
            // e.g. "spada_dacciaio_della_manticora" contains "manticora" and "acciaio"
            return normBaseName.includes(item.schoolInfo.school) && normBaseName.includes(item.schoolInfo.type);
        });
    }
    ...
    if (match) {
        ...
        if (targetDir.includes('_review_orphans')) {
            ...
            } else if (itemName.includes('spada') || itemName.includes('arco') || itemName.includes('balestra') || itemName.includes('pugnale') || itemName.includes('lupo') || itemName.includes('grifone') || itemName.includes('gatto') || itemName.includes('orso') || itemName.includes('vipera') || itemName.includes('manticora') || itemName.includes('lumaca')) {
        ...
        if (f.name === match.fileName && f.path === targetPath) {
            ...
            try {
                ...
                if (fs.existsSync(targetPath) && f.path !== targetPath) {
                    // Check which one is larger (better data usually)
                    const existingSize = fs.statSync(targetPath).size;
                    const newSize = fs.statSync(f.path).size;
                    if (newSize > existingSize) {
                        console.log(` Overwriting smaller file (${existingSize}b) with larger one (${newSize}b)`);
                        fs.unlinkSync(targetPath);
                        fs.renameSync(f.path, targetPath);
                    } else {
                        console.log(` Skipping, existing file is better/equal size`);
                        // Still mark as found but don't move/rename this specific duplicate
                    }
                } else {
                    fs.renameSync(f.path, targetPath);
                }
...
// Move remaining (non-processed) to orphans if not already there
actualFiles.forEach(f => {
    if (!processedFiles.has(f.path) && !f.path.includes('_review_orphans')) {
...
// 5. Final Report
fs.writeFileSync('e:/AntigravitiProgetti/CompendioTheWitcher/scratch/sanitization_report_v4.json', JSON.stringify(report, null, 2));

console.log(`\nSanitization V4 Complete:`);
